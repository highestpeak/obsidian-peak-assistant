...
const streamdownHostFile = path.join(root, "src/styles/streamdown-shadow-host.css");
const isolatedCssOutFile = path.join(root, "src/styles/streamdown-isolated-css.ts");
...
/** Write streamdown-isolated-css.ts for Shadow DOM injection (host + streamdown + KaTeX). */
function writeIsolatedCssTs() {
  setupKatexFonts();
  const hostCss = safeRead(streamdownHostFile);
  const streamdownCss = safeRead(streamdownFile);
  let katexCss = safeRead(katexFile);
  katexCss = fixKatexPaths(katexCss);
  const combined =
    (hostCss ? hostCss.trim() + "\n" : "") +
    streamdownCss +
    "\n/* KaTeX */\n" +
    katexCss;
  const escaped = escapeForTemplateLiteral(combined);
  const tsContent = `/** Generated by scripts/concat-css.mjs - do not edit directly. */
export const STREAMDOWN_ISOLATED_CSS = \`${escaped}\`;
`;
  const dir = path.dirname(isolatedCssOutFile);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  fs.writeFileSync(isolatedCssOutFile, tsContent, "utf8");
}
...
if (mode === "watch") {
  watch([tailwindFile, streamdownFile, streamdownHostFile, katexFile]);
} else {
  writeCombined();
}
