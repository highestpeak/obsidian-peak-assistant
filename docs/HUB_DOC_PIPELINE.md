...
- **Auto-generated hubs**: maintenance still materializes `Hub-{hash}.md` under `{hubSummaryFolder}/` (not inside `Manual/`). YAML frontmatter keeps a small identity set (`type`, `source_kind`, `source_path`, `source_node_id`, `hub_role`, `peak_auto_hub`, `peak_user_owned`, `generated_at`). Scores, assembly hints, and member lists live in a final `# Hub Metadata` section as a ```json code block (see `buildHubDocMarkdown` in `src/service/search/index/helper/hub/hubDocServices.ts`). Hub LLM prompts strip that section from the draft body preview.
