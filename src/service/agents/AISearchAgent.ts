...
export class AISearchAgent {
    ...
    private updateAgentResultTool(): AgentTool {
        /**
         * Normalize text into a stable id key (used by concept/tag ids).
         * Keeps the output ASCII-ish and avoids whitespace/symbol variations causing duplicates.
         */
        ...
        const config: UpdateResultToolConfig = {
            availableFields: [
                ...
                },
                {
                    ...
                    description: 'Dynamic blocks: inspiration tiles, markdown, action groups, Mermaid. Each has id, title/category, slot (MAIN|SIDEBAR|FLOW), renderEngine (TILE|MARKDOWN|ACTION_GROUP|MERMAID), and content (items/markdown/mermaidCode)',
                    type: 'array'
                }
            ],
            itemSchemas: {
                ...
                'graph.nodes': z.object({
                    ...
                    label: z.string().default(DEFAULT_PLACEHOLDER),
                    /**
                     * For document/file nodes: actual vault-relative file path.
                     * For concept/tag nodes: SHOULD be omitted.
                     */
                    ...
                    // Ensure path is meaningful for document/file nodes (and only those).
                    if (type === 'document' || type === 'file') {
                        const attrsPath = (data as any)?.attributes?.path;
                        if ((!data.path || data.path === DEFAULT_PLACEHOLDER || data.path === 'Untitled') && attrsPath) {
                            (data as any).path = attrsPath;
                        }

                        if (!data.path || data.path === DEFAULT_PLACEHOLDER || data.path === 'Untitled') {
                            ctx.addIssue({
                                code: z.ZodIssueCode.custom,
                                message: "Document/file nodes must have a valid path. Provide the actual vault-relative file path (e.g., 'folder/my-file.md').",
                                path: ["path"]
                            });
                            return;
                        }

                        // Stable id for document/file nodes.
                        ...
                        if (!id || id === DEFAULT_PLACEHOLDER || id.startsWith('node:') || id.startsWith('src:')) {
                            (data as any).id = expectedId;
                        } else if (id.startsWith('file:')) {
                            // Normalize potential leading slashes in file ids.
                            (data as any).id = toFileNodeId(id.slice('file:'.length));
                        }
                    } else if (type === 'concept' || type === 'tag') {
                        // Concept/tag nodes should NOT carry a file path.
                        if (data.path === DEFAULT_PLACEHOLDER || data.path === 'Untitled') {
                            (data as any).path = undefined;
                        }
                        ...
                        if (!rawLabel || rawLabel === DEFAULT_PLACEHOLDER) {
                            ctx.addIssue({
                                code: z.ZodIssueCode.custom,
                                message: "Concept/tag nodes must have a non-empty label or title.",
                                path: ["label"]
                            });
                            return;
                        }
                        ...
                        const id = String((data as any).id ?? '').trim();
                        if (!id || id === DEFAULT_PLACEHOLDER || id.startsWith('node:')) {
                            (data as any).id = expectedId;
                        } else if (id.toLowerCase().startsWith('concept:')) {
                            (data as any).id = `concept:${normalizeSpecialKey(id.slice('concept:'.length))}`;
                        } else if (id.toLowerCase().startsWith('tag:')) {
                            (data as any).id = `tag:${normalizeSpecialKey(id.slice('tag:'.length))}`;
                        }
                    }
            ...
            examples: [
                ...
                'Add dashboard block: {"operations": [{"operation": "add", "targetField": "dashboardBlocks", "item": {"id": "block1", "title": "Key Finding", "slot": "MAIN", "renderEngine": "TILE", "items": [{"id": "item1", "title": "Insight", "description": "Found important pattern", "icon": "bulb", "color": "yellow"}}]}]}'
            ],
        ...
        return {
            ...
            execute: async (input: any) => {
                // Snapshot graph before mutation. If validation fails, we will rollback
                // to avoid leaving the UI with dangling edges (non-existent endpoints).
                const graphSnapshot = this.cloneGraphForRollback();

                // Execute the original tool
                const result = await baseTool.execute(input);

                // Normalize IDs/endpoints before running consistency checks.
                // This reduces common LLM mistakes like mixing "src:" with "file:" or using "node:Label" for concepts.
                this.normalizeAgentGraphInPlace();
                ...
                if (!graphValidation.isValid) {
                    ...
                    const correctiveMessage = `⚠️ GRAPH CONSISTENCY ISSUES DETECTED:\n${graphValidation.issues.join('\n')}\n\nPlease fix these issues by:\n1. Remove invalid edges using: {"operations": [{"operation": "remove", "targetField": "graph.edges", "removeId": "edge-id"}]}"\n2. Add missing nodes using: {"operations": [{"operation": "add", "targetField": "graph.nodes", "item": {...}}]}\n3. Ensure all edges reference existing nodes\n\nTool execution result: ${result}`;

                    // Rollback graph mutations to keep UI state consistent.
                    this.agentResult.graph = graphSnapshot;

                    // Inject corrective reminder into agent memory for next iteration
                    const correctiveSystemMessage = buildLLMRequestMessage(
                        'system',
                        `Graph data consistency check failed. Issues: ${graphValidation.issues.join('; ')}. You must fix these issues before proceeding.`
                    );
     ...
     */
    private cloneGraphForRollback(): { nodes: any[]; edges: any[] } {
        try {
            return JSON.parse(JSON.stringify(this.agentResult.graph));
        } catch {
            return { nodes: [...this.agentResult.graph.nodes], edges: [...this.agentResult.graph.edges] };
        }
    }
     ...
     */
    private normalizeAgentGraphInPlace(): void {
        const g = this.agentResult.graph;
        const normalizeSpecialKey = (raw: unknown): string => {
            const text = String(raw ?? '').trim().toLowerCase();
            return text
                .replace(/[_\s]+/g, '-') // whitespace/underscore -> dash
