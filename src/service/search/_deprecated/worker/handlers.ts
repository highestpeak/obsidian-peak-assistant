...
export class WorkerHandlers {
	...
	async indexDocuments(payload: IndexDocumentsRequest): Promise<OkResponse> {
		...
		const graphStore = await this.ctx.getGraphStore();
		const docMetaRepo = this.ctx.getDocMetaRepo();
		const indexState = this.ctx.getIndexStateRepo();

		// Process documents: use chunkId if available, otherwise use path
		const docs = (payload?.docs ?? []).map((d: any) => ({
			id: d.chunkId ?? d.path, // Use chunkId for chunked documents, path for regular documents
			path: d.path, // Always preserve original path
			title: d.title,
			type: d.type ?? 'markdown',
			content: d.content ?? '',
			mtime: d.mtime ?? 0,
			embedding: d.embedding, // Per-chunk embedding if provided
		}));
		...
		if (docs.length) {
			...
			// For metadata, only store original documents (not chunks)
			// Group by original path to avoid duplicate metadata entries
			const originalDocs = new Map<string, { path: string; title: string; type: string; mtime: number }>();
			for (const d of docs) {
				// If this is a chunk, use original path; otherwise use path as-is
				const originalPath = d.path;
				if (!originalDocs.has(originalPath)) {
					originalDocs.set(originalPath, {
						path: originalPath,
						title: d.title,
						type: d.type,
						mtime: d.mtime,
			...
			for (const doc of originalDocs.values()) {
				docMetaRepo.upsert(doc);
			}
			
			// For graph, only process original documents (not chunks)
			// Group chunks by original path and combine content
			const graphDocs = new Map<string, { content: string; docId: string; docType: string }>();
			for (const d of docs) {
				const originalPath = d.path;
				if (!graphDocs.has(originalPath)) {
					// Extract document ID from chunk ID if available, otherwise use path
					const docId = d.id.includes(':chunk:') 
						? d.id.split(':chunk:')[0] 
						: (d.id === originalPath ? originalPath : d.id);
					graphDocs.set(originalPath, { 
						content: '', 
						docId: docId,
						docType: d.type 
					});
				}
				// Combine chunk contents (simplified - in practice might want better merging)
				const existing = graphDocs.get(originalPath)!;
				graphDocs.set(originalPath, {
					...existing,
					content: existing.content ? existing.content + '\n\n' + d.content : d.content,
				});
			}
			for (const [path, docData] of graphDocs.entries()) {
				if (docData.docType === 'markdown') {
					graphStore.upsertMarkdownDocument({
						id: docData.docId,
						path: path,
			...
			indexState.set(INDEX_STATE_KEYS.indexedDocs, String(prevCount + originalDocs.size));
		}
		return { ok: true } as const;
	}
	...
	async deleteDocuments(payload: DeleteDocumentsRequest): Promise<OkResponse> {
		...
		for (const p of paths) {
			// Use path as document ID for backward compatibility
			graphStore.removeDocument(p);
		}
		return { ok: true } as const;
	}
	...
	async exportStorage(payload: ExportStorageRequest): Promise<ExportStorageResponse> {
		const types = payload?.types ?? ['sqlite', 'orama'];
		...
		if (types.includes('sqlite')) {
			...
			// Graph is now stored in SQLite, so no need for separate export
		}
