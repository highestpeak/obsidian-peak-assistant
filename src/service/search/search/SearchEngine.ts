...
import type { OramaSearchIndex } from '@/service/storage/orama/OramaSearchIndex';
import type { RecentOpenRepo } from '@/service/storage/sqlite/repositories/RecentOpenRepo';
import type { SearchGraphIndex } from '@/service/storage/graph/SearchGraphIndex';
 ...
 */
function buildAiAnalyzeResult(params: { hits: any[]; query: string; graph: SearchGraphIndex }): AiAnalyzeResult {
	const sources = buildRagSources({ hits: params.hits, query: params.query });
	const focusPath = sources[0]?.path;
	const graphPreview = focusPath ? params.graph.getPreview({ currentFilePath: focusPath, maxNodes: 28 }) : { nodes: [], edges: [] };
...
export class SearchEngine {
	constructor(
		...
		private readonly graph: SearchGraphIndex,
	) {}
	...
	async search(params: { query: SearchQuery }): Promise<SearchResponse> {
		...
		const related = scope?.currentFilePath ? this.graph.getRelatedFilePaths({ currentFilePath: scope.currentFilePath, maxHops: 2 }) : new Set<string>();
	...
	async aiAnalyze(params: { req: AiAnalyzeRequest; embedding?: number[] }): Promise<AiAnalyzeResult> {
		...
		return buildAiAnalyzeResult({ hits: results?.hits ?? [], query: q, graph: this.graph });
	}
}
