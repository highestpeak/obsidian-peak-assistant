 ...
 *
 * Pure function — no SQLite, no side effects. The caller supplies a sync
 * `getOutEdges` callback that returns weighted out-edges for a given node.
 * This will be made async in the edge-fetcher integration (Task 4).
 */
 ...
 * @param getOutEdges - Sync callback returning weighted out-edges for a node.
...
export function computePPR(
	...
	function getCachedOutEdges(nodeId: string): MultiLayerEdge[] {
	...
	while (true) {
		...
		for (const [v, rv] of r) {
			if (rv <= 0) continue;
			const edges = getCachedOutEdges(v);
		...
		const edges = getCachedOutEdges(bestNode);
