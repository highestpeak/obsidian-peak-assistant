...
export const V2ContinueAnalysisInput: React.FC<V2ContinueAnalysisInputProps> = ({ onClose }) => {
    ...
    return (
        <>
            ...
            >
                <div className="pktw-bg-white pktw-border pktw-border-[#e5e7eb] pktw-rounded-xl pktw-shadow-lg pktw-overflow-hidden">
                    {/* Header */}
                    <div className="pktw-flex pktw-items-center pktw-justify-between pktw-px-4 pktw-py-3 pktw-border-b pktw-border-[#e5e7eb]">
                        <span className="pktw-text-xs pktw-font-semibold pktw-text-[#2e3338]">
                            Continue Analysis
                        </span>
                        <div
                            onClick={onClose}
                            className="pktw-text-[#9ca3af] hover:pktw-text-[#6b7280] pktw-transition-colors pktw-cursor-pointer"
                        >
                    ...
                    <div className="pktw-p-4">
                        <div className="pktw-flex pktw-items-center pktw-gap-3 pktw-mb-3">
                            <input
                                ...
                                className="pktw-flex-1 pktw-px-3 pktw-py-2 pktw-text-sm pktw-border pktw-border-[#e5e7eb] pktw-rounded-lg pktw-outline-none focus:pktw-ring-2 focus:pktw-ring-[#7c3aed]/50 focus:pktw-border-transparent"
                            />
                            <div
                                onClick={() => handleSubmit()}
                                className={`pktw-flex-none pktw-p-2 pktw-rounded-lg pktw-transition-colors pktw-cursor-pointer ${
                                    input.trim()
                                        ? 'pktw-text-white pktw-bg-[#7c3aed] hover:pktw-bg-[#6d28d9]'
                                        : 'pktw-text-[#9ca3af] pktw-bg-gray-100'
                                }`}
                            >
                        ...
                        {suggestions.length > 0 && (
                            <div className="pktw-space-y-2">
                                <span className="pktw-text-xs pktw-text-[#9ca3af]">Suggested questions:</span>
                                <div className="pktw-flex pktw-flex-wrap pktw-gap-2">
                                    {suggestions.slice(0, 4).map((s, i) => (
                                        <motion.div
                                            ...
                                            className="pktw-px-3 pktw-py-1.5 pktw-text-xs pktw-text-[#6b7280] pktw-bg-gray-100 hover:pktw-bg-purple-100 hover:pktw-text-[#7c3aed] pktw-rounded-full pktw-transition-colors pktw-cursor-pointer"
                                        >
                                            {s}
                                        </motion.div>
                                    ))}
