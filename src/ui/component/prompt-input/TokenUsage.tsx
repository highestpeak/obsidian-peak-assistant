...
export const TokenUsage: React.FC<TokenUsageProps> = ({ usage, conversation, className }) => {
	...
	// Create hover menu content for detailed usage
	const menuContent = useMemo(() => {
		if (!hasDetails) return null;

		return (
			<div className="pktw-space-y-3 pktw-min-w-[300px] pktw-max-w-md">
	...
	}, [hasDetails, groupedUsage, usage.totalUsed]);

	if (!hasDetails) {
		return (
			<div
				className={cn(
					'pktw-inline-flex pktw-items-center pktw-justify-center pktw-h-8 pktw-px-2.5 pktw-rounded-md pktw-text-xs pktw-font-medium pktw-bg-gray-100 pktw-text-[#22c55e]',
					className
				)}
			>
